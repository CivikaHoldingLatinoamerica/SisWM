...
class Catalogos extends CI_Controller
{
	...
	/**
	 * apartado de catalogo nacional de ocupaciones
	 * es referente al crud pero que no estara habilitado para la postperidad
	 */
	public function nacional_ocupaciones(){
		try {
			$data['titulo_pagina'] = 'Catalogo nacional de ocupaciones';
			$data['migas_pan'] = array(
				array('nombre' => 'Inicio', 'activo' => false, 'url' => base_url()),
				array('nombre' => 'Catalogos', 'activo' => false, 'url' => '#'),
				array('nombre' => 'Nacional de ocupaciones', 'activo' => true, 'url' => '#'),
			);
			$data['sidebar'] = 'cat_ocupaciones';
			$data['usuario'] = $this->usuario;
			$data['extra_js'] = array(
				base_url() . 'assets/js/catalogos/ocupaciones.js',
			);
			$data['extra_css'] = array();
			$this->load->model('CatOcupacionEspecificaModel');
			$data['cat_ocupacion_especifica'] = $this->CatOcupacionEspecificaModel->catalogo();
			//var_dump($data);exit;
			$this->load->view('catalogo/ocupaciones', $data);
		} catch (Exception $ex) {
			$response['success'] = false;
			$response['msg'][] = 'Hubo un error en el sistema, intente nuevamente';
			$response['msg'][] = $ex->getMessage();
	...
	public function agregar_modificar_ocuaciones(){
		try{
			$post = $this->input->post();
		}catch(Exception $ex){
			$response['success'] = false;
