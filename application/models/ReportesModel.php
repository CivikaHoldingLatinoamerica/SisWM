...
class ReportesModel extends ModeloBase
{
	...
	public function obtener_reporte_empresa($para_vista = false){
		try{
			$consulta = "select 
				de.nombre nombre_empresa,
				upper(de.rfc) as rfc_empresa,
				'Inscrita' as estatus_inscripcion,
				if(count(uhec.id_usuario_has_estandar_competencia) = count(uhec.jucio_evaluacion = 'competente' ),'Finalizada','Iniciada' ) as proceso_evaluacion,
