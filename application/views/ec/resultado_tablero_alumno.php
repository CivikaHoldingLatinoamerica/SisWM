<?php if (isset($estandar_competencia) && sizeof($estandar_competencia) != 0): ?>
	<?php foreach ($estandar_competencia as $index => $ec): ?>
		<?php if(isset($ec->instructor) && $ec->instructor!=null): ?>
	...
	<!--					<h3 class="widget-user-desc text-center" ><?=$ec->codigo?></h3>-->
					...
					<div class="card-body">
						<div class="row from-group " >
							<div class="col-12 centrado">
								<img class="img-circle img-perfil-usuario-tablero" src="<?=isset($ec->instructor->foto_perfil) ? $ec->instructor->foto_perfil : base_url().'assets/imgs/iconos/admin.png' ?>"
									alt="<?=isset($ec->instructor) && $ec->instructor!=null ? $ec->instructor->nombre.' '.$ec->instructor->apellido_p : 'Instructor Asignado'?>">
							</div>
						</div>
						<div class="row text-justify">
							...
							<?php endif ?>
							<p>
								<b>Evaluador: </b><?=isset($ec->instructor) && $ec->instructor!=null ? $ec->instructor->nombre.' '.$ec->instructor->apellido_p.' '.$ec->instructor->apellido_m : 'Instructor Asignado'?>
							</p>
						</div>
					</div>
